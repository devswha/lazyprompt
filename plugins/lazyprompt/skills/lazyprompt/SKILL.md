...
- 발동 방식(OmX 방식): 아이디어의 **막연도를 먼저 가늠**해서 **명확하면 질문 없이 바로 2부 출력**, **막연하면 자동으로 딥 인터뷰**(기본 quick, 짧게)로 들어간다. 스킵/조절 오버라이드 — `--direct`나 "그냥 만들어"/"바로"/"묻지 말고"면 막연해도 인터뷰 없이 바로, `--interview`/`--quick`/`--standard`/`--deep`(또는 "인터뷰해줘"류)면 강제로 켜고 깊이 지정. 자세한 건 아래 "딥 인터뷰 모드".
...
5. 딥 인터뷰(모호도 점수·양방향 임계값 게이트)는 **막연할 때 자동 발동**하되 `--direct`/"그냥 만들어"로 **언제든 스킵**할 수 있다. 인터뷰를 켜도 프롬프트 텍스트만 만들고 타깃 스킬을 실행하지 않는다.
6. **명확한 입력**에서는 인터뷰 없이 바로 출력한다. 인터뷰를 돌 때도 고정 문항·형식적 확인으로 턴을 낭비하지 않고, 가장 약한 곳만 물어 최소 라운드로 수렴한다.

## 동작 순서 (Operating sequence)
0. **라우팅(막연도 게이트, OmX 방식):** 먼저 아이디어의 막연도를 가늠한다.
   - **명확 신호**(구체 파일·심볼·이슈/PR 번호·에러·번호 매긴 단계·성공기준이 있거나 WHAT+완료기준이 뚜렷): 인터뷰 없이 아래로 진행해 **바로 2부 출력**.
   - **막연 신호**(모호한 동사만·구체 근거(anchor) 없는 짧은 한 줄·3개+ 영역·핵심 명사가 흔들림): **자동으로 딥 인터뷰**(기본 quick)로 들어간다.
   - **스킵 오버라이드**: `--direct` 또는 "그냥 만들어"/"바로"/"묻지 말고"/"프롬프트만"이면 막연해도 인터뷰 없이 바로 만들되, 모르는 값은 `사용자 미확인`으로 남긴다.
   - **강제/깊이 오버라이드**: `--interview`/`--quick`/`--standard`/`--deep`나 "인터뷰해줘"류면 명확해 보여도 인터뷰를 켜고 깊이를 지정한다.
1. 사용자 아이디어를 읽고 **plan vs loop를 추론**한다.
   - plan 추천: 무엇을 만들지·범위·아키텍처·실행계획이 아직 결정 전일 때.
   - loop 추천: 만들 결과가 비교적 명확하고 반복 실행·검증·완료 판정이 필요할 때.
2. (인터뷰를 안 타는 경로) 타깃을 합리적으로 추론할 수 있으면 **질문 없이 바로 출력**한다. 추천 이유와 불확실성은 `프롬프트 설명`에 적는다.
3. 바로 출력 경로인데 핵심 정보가 살짝 부족하면(막연 신호까진 아님) 질문은 **한 번에 모두 묶어서 묻고**, 모르면 건너뛰어도 된다고 안내한다. 깊게 막연하면 여기서 묻지 말고 0번의 자동 딥 인터뷰로 보낸다.
...
- **강제·깊이 지정**: `--interview`(켜기), `--quick`/`--standard`/`--deep`(깊이). 자연어 "인터뷰해줘", "하나씩 물어봐", "꼼꼼히 물어봐", "deep interview", "ouroboros"도 강제로 켠다.
  - 예: `$lazyprompt --deep 우리 서비스에 검색 기능 넣고 싶어`
- **스킵**: `--direct` 또는 "그냥 만들어"/"바로"/"묻지 말고"/"프롬프트만"이면 막연해도 인터뷰 없이 바로 만든다(모르는 값은 `사용자 미확인`).
...
자동 발동 시 기본은 **quick(0.60)**. `--interview`만 주고 깊이를 안 주면 standard(0.50)로 본다.
