...
스킬이 `plan` vs `loop`를 추천하고, 3~5개만 물어본 뒤, 그대로 복붙할 수 있는 프롬프트를 뱉습니다.
...
1. **추천 + 확인** — 아이디어가 `ulw-plan`(계획)용인지 `ulw-loop`(실행)용인지 추천하고, 당신이 확정.
2. **가벼운 질문 3~5개** — 점수·게이트 없이 핵심만.
...
스킬이 `loop`를 추천·확인하고 몇 가지를 물은 뒤, 아래 형태로 출력합니다:
