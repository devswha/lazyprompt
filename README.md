...
- **막연하면 알아서 물어봅니다(OmX 방식).** 아이디어가 뚜렷하면 바로 프롬프트, 막연하면 자동으로 딥 인터뷰(짧게)로 들어가 가장 약한 부분을 하나씩 물어 채운 뒤 만듭니다. 그냥 바로 받고 싶으면 `--direct` 또는 "그냥 만들어", 더 깊게 잡고 싶으면 `--deep`.
...
### 딥 인터뷰 (막연하면 자동, GJC deep-interview 이식)

막연한 아이디어를 **한 번에 하나씩** 물어 명료도를 점수로 끌어올린 뒤 프롬프트를 만드는 모드. **막연하면 자동으로 켜지고(기본 짧게), 명확하면 안 켜지고 바로 출력**합니다. 스킵/조절은 아래 오버라이드로.

```bash
$lazyprompt 우리 서비스에 검색 기능 넣고 싶어         # 막연 → 자동으로 짧은 인터뷰
$lazyprompt --direct 로그인 API 만들어줘             # 막연해도 인터뷰 건너뛰고 바로
$lazyprompt --deep 우리 서비스에 검색 기능 넣고 싶어   # 더 깊게 강제 (--interview/--quick/--standard도)
```
